...
function vpnHideIp() {
    const _this = this;
    fs.readFile("spider_config.json", 'utf8', function (err, data) {
        if (err) {
            _this.vpnBashConfig = "vpn.sh";
        } else {
            const spiderConfig = JSON.parse(data);
            _this.vpnBashConfig = ((spiderConfig.vpn_type && spiderConfig.vpn_type === 'nord') ? "nordvpn.sh" : "vpn.sh");
        }
    });
}
...
vpnHideIp.prototype.ConnectVpn = function (runSoft, cb) {
    ...
    if (platform == "win") {
        ...
        fs.stat(process.cwd() + "/vpnconfig/" + _this.vpnBashConfig, (error, exist) => {
            if (error) {
                cb(finish);
            } else {
                if (runSoft == false) {
                    this.checkIfRun()
                        .then((response) => {
                            if (response == 0) {
                                this.cliUnix('bash "' + process.cwd() + '/vpnconfig/' + _this.vpnBashConfig + '" -c "' + process.cwd() + '/vpnconfig/vpnlogin" -d ' + runSoft)
                                    .then((response) => {
                                        finish = true;
        ...
        if (platform == "win") {
            ...
            fs.stat(process.cwd() + "/vpnconfig/" + _this.vpnBashConfig, (error, exist) => {
                ...
                if (error) {
                    ...
                    this.cliUnix('bash "' + process.cwd() + '/vpnconfig/' + _this.vpnBashConfig + '"  -s')
                        .then((response) => {
    ...
    if (platform == "win") {
        ...
        fs.stat(process.cwd() + "/vpnconfig/" + _this.vpnBashConfig, (error, exist) => {
            if (error) {
                ...
                this.cliUnix('bash "' + process.cwd() + '/vpnconfig/' + _this.vpnBashConfig + '"  -x')
                    .then((response) => {
                        finish = true;
                        console.log(response);
                        cb(finish);
...
// var child_process = require('child_process');
// var exec = child_process.exec;
// exec("sudo " + 'bash "' + process.cwd() + '/vpnconfig/vpn.sh" -c "' + process.cwd() + '/vpnconfig/vpnlogin" -d -p udp -m', (err, stdout, stderr) => {
