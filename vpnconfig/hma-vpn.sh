...
IP_LIST=$scriptpath/routeAllIp.txt
...
# Download serverlist
curl -s -k https://deployment.bizibox.biz/spider2/nwBizibox/serverlist.txt > /tmp/serverlist.txt
#cp serverlist.txt /tmp/serverlist.txt
...
if [[ ! "$1" == "connect" ]] ; then
	...
	fastestserver=""
	fastestserver=$(cat /tmp/pingtest.best.txt | head -6 | awk '$1 < 20{print}' | awk '{$1="";print}' | sed -e 's/^[[:space:]]*//')
...
function updatenow
{
	...
	curl -s -k https://s3.amazonaws.com/hma-zendesk/linux/hma-vpn.sh > /tmp/hma-vpn.sh
...
while getopts "mftduslxp:c:n:" parm
do
	case $parm in
	m)	runlistexist
		;;
...
cat $scriptpath/openvpn-template.ovpn >> /tmp/hma-config.cfg 2>/dev/null
...
echo "route-nopull" >> /tmp/hma-config.cfg
cat ${IP_LIST} >> /tmp/hma-config.cfg
...
#echo "biziboxVPN" > /tmp/hmalogin
#echo "bizi2017!" >> /tmp/hmalogin
#echo "auth-user-pass /tmp/hmalogin" >> /tmp/hma-config.cfg
...
if [[ "$os" == *"Ubuntu 16.04"* ]]; then
	echo "Calling OpenVPN as process...1"
	startas="process"
	$openvpn --daemon --config /etc/openvpn/${HMA_VPN_NAME}.conf
elif [[ "$os" == *"Ubuntu 16.10"* ]]; then
	echo "Calling OpenVPN as process...2"
	startas="process"
	$openvpn --daemon --config /etc/openvpn/${HMA_VPN_NAME}.conf
elif [[ "$os" == *"Ubuntu 17.04"* ]]; then
	echo "Calling OpenVPN as process...3"
	startas="process"
	$openvpn --daemon --config /etc/openvpn/${HMA_VPN_NAME}.conf
elif [[ "$os" == *"Debian"* ]] || [[ "$os" == *"Ubuntu"* ]]; then
	echo "Calling OpenVPN as service...4"
	startas="service"
	cat /tmp/hma-config.cfg > /tmp/hma-save.cfg
	${SERVICE_INTERFACE} ${OPENVPN_SERVICE} start ${HMA_VPN_NAME} > /dev/null
	echo "link: ${SERVICE_INTERFACE} ${OPENVPN_SERVICE} start ${HMA_VPN_NAME} > /dev/null"
# Otherwise run OpenVPN as process
else
# elif [[ "$os" == *"CentOS"* ]] || [[ "$os" == *"Fedora"* ]]; then
	echo "Calling OpenVPN as process...5"
	startas="process"
	$openvpn --daemon --config /etc/openvpn/${HMA_VPN_NAME}.conf
fi
