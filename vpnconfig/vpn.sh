...
VPN_NAME=EXPRESSVPN
VPN_STATUS_FILE=/tmp/vpn-status.txt
VPN_LOG_FILE=/tmp/vpn-log.txt
...
function cleanup {
  ...
  unset TAILERPID
  rm /tmp/vpn-config.cfg 2>/dev/null
  rm /tmp/ipcheck.sh 2>/dev/null
...
function pingtest {
  ...
  servercount=$(wc -l serverlist.txt | awk '{print $1}')
  i=1
...
if [[ $isisrael -eq 0 ]]; then
  ...
  i=0
  ips[0]="israel-ca-version-2.expressnetw.com"
  SERVER="${ips[$i]}"
  echo "Selected Server:"
  echo -e $SERVER
fi
...
cat $scriptpath/openvpn-template.ovpn >>/tmp/vpn-config.cfg 2>/dev/null

# Add a few config lines we'll need
echo "remote ${ips[$i]} $port" >>/tmp/vpn-config.cfg
echo "log-append ${VPN_LOG_FILE}" >>/tmp/vpn-config.cfg
echo "route-up /tmp/routeup.sh" >>/tmp/vpn-config.cfg
echo "down /tmp/down.sh" >>/tmp/vpn-config.cfg

# If credentials file was specified, add location to config file
if [ "$authfile" ]; then
  echo "auth-user-pass $authfile" >>/tmp/vpn-config.cfg
fi
...
#nohup /tmp/ipcheck.sh >/dev/null 2>&1 &
nohup /tmp/ipcheck.sh &
rm /tmp/routeup.sh
EOF
